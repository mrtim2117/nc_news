...
This will install development and production dependencies:
...
At the start of each test run, all tables are dropped from the test database, recreated and re-populated with fresh test data. All tests are defined in the `app.test.js` file, and are executed by issuing the following command from the root project folder:
